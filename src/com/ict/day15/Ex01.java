...
/*
 	...
 	컬렉션 : 객체들을 모아서 저장 관리하는 인터페이스들을 말한다.
 		...
 		저장적인 측면으로 보기
 		변수 => 하나의 데이터를 저장할 수 있는 메모리 공간
 		배열 => 같은 종류의 자료형을 저장
 			   크기를 먼저 지정해야 된다. 변경이 안된다.
 			   변수와 객체를 여러개를 저장 할 수 있다.
 		클래스 => 배열은 같은 종류의 자료형을 저장하는 것이 불편해서 클래스에 저장하면
 				다양한 종류의 데이터를 저장 할 수 있다. 
 		컬랙션 => 다양한 종류의 데이터를 저장한 클래스를 하나의 객체로 만들고
 				그 객체를 컬렉션에 저장한 후 관리까지 할 수 있는 인터페이스
 		
 	컬렉션 형식 : 컬렉션<제네릭 타입>
 	
 	API 용어들 
 	1. <T> => 객체 타입(Type), <E> => 요소(Element):컬랙션이 가지고 있는 객체 하나하나를 뜻함
 	2. Map 형식 용어 : <K,V> => Key(키값), Value(데이터값)
 			특징) 1:1 대응(K 하나은 V 하나만 가능), **Key를 호출하면 Value가 나온다.(변수와 비슷)
 	
 	최상위 컬렉션 2가지 : Collection<E>, Map<K,V>
 	Collection<E>를 상속 받은 인터페이스들 : Set(E), List(E), Queue(E)
 	
 	Collection의 주요 메서드
 	add(E e) : boolean => 해당 컬렉션에 인자로 받은 요소(E)를 추가, 성공하면 true, 실패하면 false
 	...
 	**iterrator() : Iterrator<E> => 컬렉션 안에 존재하는 객체들을 순서대로 접근하기 위해서
 									Iterator 객체를 반환 
